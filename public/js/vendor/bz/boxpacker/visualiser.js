...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    PACKING_SIMULATION.forEach((element) => {
        const PACKING = element;
        ...
        const createScene = () => {
            ...
            for (let packedBoxKey in packingData) {
                showAxis(
                    ZOOM * packingData[packedBoxKey].box.innerWidth,
                    ZOOM * packingData[packedBoxKey].box.innerLength,
                    ZOOM * packingData[packedBoxKey].box.innerDepth,
                    boxPlacementX
                );
                ...
                box.position.x =
                    boxPlacementX +
                    (ZOOM * packingData[packedBoxKey].box.innerWidth) / 2;
                box.position.z =
                    (ZOOM * packingData[packedBoxKey].box.innerLength) / 2;
                box.position.y =
                    (ZOOM * packingData[packedBoxKey].box.innerDepth) / 2;
                ...
                for (let itemsKey in packingData[packedBoxKey].items) {
                    let item = BABYLON.MeshBuilder.CreateBox(
                        "item" + itemsKey,
                        {
                            width:
                                ZOOM *
                                packingData[packedBoxKey].items[itemsKey].width,
                            depth:
                                ZOOM *
                                packingData[packedBoxKey].items[itemsKey]
                                    .length,
                            height:
                                ZOOM *
                                packingData[packedBoxKey].items[itemsKey].depth,
                        }
                    );
                    ...
                    item.position.x =
                        ...
                        (ZOOM *
                            packingData[packedBoxKey].items[itemsKey].width) /
                            2 +
                        ZOOM * packingData[packedBoxKey].items[itemsKey].x;
                    item.position.z =
                        (ZOOM *
                            packingData[packedBoxKey].items[itemsKey].length) /
                            2 +
                        ZOOM * packingData[packedBoxKey].items[itemsKey].y;
                    item.position.y =
                        (ZOOM *
                            packingData[packedBoxKey].items[itemsKey].depth) /
                            2 +
                        ZOOM * packingData[packedBoxKey].items[itemsKey].z;
                    ...
                    text1.text =
                        "Box: " + packingData[packedBoxKey].box.reference;
                    text1.text += "\n";
                    text1.text +=
                        "Item: " +
                        packingData[packedBoxKey].items[itemsKey].item
                            .description;
                    text1.text += "\n";
                    text1.text +=
                        "As specified (W×L×D): " +
                        packingData[packedBoxKey].items[itemsKey].item.width +
                        "×" +
                        packingData[packedBoxKey].items[itemsKey].item.length +
                        "×" +
                        packingData[packedBoxKey].items[itemsKey].item.depth;
                    text1.text += "\n";
                    text1.text +=
                        "As packed (W×L×D): " +
                        packingData[packedBoxKey].items[itemsKey].width +
                    ...
                    text1.text += "\n";
                    text1.text +=
                        "x: " + packingData[packedBoxKey].items[itemsKey].x;
                    text1.text += "\n";
                    text1.text +=
                        "y: " + packingData[packedBoxKey].items[itemsKey].y;
                    text1.text += "\n";
                    text1.text +=
                        "z: " + packingData[packedBoxKey].items[itemsKey].z;
                    text1.color = "White";
                    text1.fontSize = 14;
                ...
                boxPlacementX +=
                    2 * ZOOM * packingData[packedBoxKey].box.innerWidth;
            }
            ...
            const camera = new BABYLON.ArcRotateCamera(
                ...
                -0.8 * Math.PI,
                0.5 * Math.PI,
                ZOOM * packingData[0].box.innerDepth * 2.5,
                new BABYLON.Vector3(
                    0,
                    (ZOOM * packingData[0].box.innerDepth) / 2,
                    0
                ),
                scene
            );
        ...
        elCard.className = "box box-body";

        const elTitle = document.createElement("h2");
        elTitle.textContent = `Master Box: ` + PACKING.box.reference;

        const canvas = document.createElement("canvas");

        elCard.appendChild(elTitle);
        elCard.appendChild(canvas);

        const elItemsTitle = document.createElement("h3");
        elItemsTitle.textContent = "Items";

        const elItems = document.createElement("div");
        elItems.style.display = "grid";
        elItems.style.gridTemplateColumns = "auto auto auto";

        let box_content = [];

        PACKING.items.forEach((x) => {
            if (
                box_content.some(
                    (val) => val["sku"] == x["item"]["description"]
                )
            ) {
                box_content.forEach((k) => {
                    if (k["sku"] === x["item"]["description"]) {
                        k["qty"]++;
                    }
                });
            } else {
                let a = {};
                a["sku"] = x["item"]["description"];
                a["qty"] = 1;
                box_content.push(a);
            }
        });

        box_content.forEach(function (item, index) {
            const elItem = document.createElement("div");
            elItem.style.display = "inline-grid";

            const elItemTitle = document.createElement("h4");
            elItemTitle.textContent = ` ${item.sku} (× ${item.qty})`;
        ...
        elCard.appendChild(elItemsTitle);
        elCard.appendChild(elItems);

        simulation_canvas.appendChild(elCard);
